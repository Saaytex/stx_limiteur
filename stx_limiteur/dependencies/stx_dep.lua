...
function PMenu:resetMenu()
	...
	self.Base = {
		...
		Title = PCore and PCore.user and PCore.user.name or "Menu",
...
function PMenu:OpenMenu(stringName, boolBack)
	...
	if stringName and not self.Menu[stringName] then print("[PMenu] " .. stringName .. " (erreur)") return end
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	local stringPrefix = (((button.r and (((PCore and (PCore.jobRank < button.r or PCore.copRank < button.r)) or (button.rfunc and not button.rfunc())) and "~r~" or "")) or "") .. (self.Events["setPrefix"] and self.Events["setPrefix"](button, self.Data) or "")) or ""
